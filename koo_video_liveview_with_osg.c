...
static HD_RESULT set_cap_param(HD_PATH_ID video_cap_path, HD_DIM *p_dim)
{
	...
	{//select sensor mode, manually or automatically
		...
		video_in_param.sen_mode = 4; //ooSSoo auto select sensor mode by the parameter of HD_VIDEOCAP_PARAM_OUT
...
static int init_novatek_logo(unsigned short *novatek_logo, int idx)
{
	...
	if(fd == -1){
		if(idx){
			...
			printf("video_frm_1000_200_colorkey_argb4444.dat is in SDK/code/hdal/samples/pattern/\n");
		}else{
...
MAIN(argc, argv)
{
	...
	UINT32 out_type = 1;
	int stamp_enable = 1, mask_enable = 1, stamp_idx = 0;
	...
	if(init_novatek_logo(stream[0].novatek_logo, stamp_idx)){
	...
	printf("Enter z to change stamp image\n");
	printf("Enter x to enable/disable stamp\n");
	printf("Enter c to enable/disable mask\n");
	while (1) {
		key = GETCHAR();
		if (key == 'q' || key == 0x3) {
			// quit program
			...
			HD_OSG_STAMP_IMG  img;
			stamp_idx = (stamp_idx ? 0 : 1);
			init_novatek_logo(stream[0].novatek_logo, stamp_idx);
			memset(&img, 0, sizeof(HD_OSG_STAMP_IMG));
			img.fmt        = HD_VIDEO_PXLFMT_ARGB4444;
			img.dim.w      = STAMP_WIDTH;
			img.dim.h      = STAMP_HEIGHT;
			img.p_addr     = (UINT32)stream[0].novatek_logo;
			if(hd_videoout_set(stream[0].stamp_path, HD_VIDEOOUT_PARAM_OUT_STAMP_IMG, &img) != HD_OK){
				printf("fail to set stamp image\n");
			}else{
				printf("stamp image changed\n");
			}
		}else if(key == 'x'){
			stamp_enable = (stamp_enable ? 0 : 1);
			if(stamp_enable){
				hd_videoout_start(stream[0].stamp_path);
				printf("stamp is enabled\n");
			}else{
				hd_videoout_stop(stream[0].stamp_path);
				printf("stamp is disabled\n");
			}
		}else if(key == 'c'){
			mask_enable = (mask_enable ? 0 : 1);
			if(mask_enable){
				hd_videoout_start(stream[0].mask_path);
				printf("mask is enable\n");
			}else{
				hd_videoout_stop(stream[0].mask_path);
